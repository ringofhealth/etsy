defmodule Etsy.TokenStore do
  ...
  def update_token(token),
    do: Agent.update(__MODULE__, fn state -> Keyword.put(state, :token, token) end)

  def update_token_secret(secret),
    do: Agent.update(__MODULE__, fn state -> Keyword.put(state, :token_secret, secret) end)
